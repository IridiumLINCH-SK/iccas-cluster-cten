...
y_train = df1['AEPA']
X_train = df1.drop(columns=['clusters', 'AEPA'])
y_val = df2['AEPA']
X_val = df2.drop(columns=['clusters', 'AEPA'])
y_test = df3['AEPA']
X_test = df3.drop(columns=['clusters', 'AEPA'])
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            for bat in train_loader:
                ...
                optimizer.step()
            end_time = time.time()
            print("epoch: {}/{}, which takes {} s.".format(epoch + 1, self.epochs, round(end_time - start_time, 1)))
        return self
